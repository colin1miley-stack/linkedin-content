...
if new_content != content:
    with open('audit.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Total investment section updated')
else:
    print('Pattern not found - trying alternative approach')
    # Try simpler string replacement
    old_simple = '''        total = bundle.get("entry_price", 0) + bundle.get("total_implementation", 0)
        html += f"""
    </table>
    <div class="highlight-box">
        <p><strong>Total Investment (à la carte):</strong> €{total:,}</p>
...
<div class="footer">'''
    ...
    new_simple = '''        if bundle.get("is_rebuild"):
            # For rebuilds: show rebuild cost only (automations included)
            rebuild_low = bundle.get("rebuild_price_low", 3500)
            rebuild_high = bundle.get("rebuild_price_high", 6500)
            monthly_recurring = sum(s.get("price", 0) for s in bundle.get("implementation_services", []) if "/mo" in s.get("price_display", ""))
...
<div class="footer">'''
    ...
    if old_simple in content:
        ...
        with open('audit.py', 'w', encoding='utf-8') as f:
            f.write(content)
        print('SUCCESS: Total investment section updated (fallback)')
    else:
        print('ERROR: Could not find replacement target')
        # Debug: show surrounding context
        idx = content.find('total = bundle.get')
        if idx >= 0:
